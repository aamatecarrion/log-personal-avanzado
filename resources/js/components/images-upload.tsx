...
import type { BreadcrumbItem, UploadLimit } from '@/types';
import { Frown, UploadCloud, X } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Progress } from '@/components/ui/progress';
import { formatFechaEspanola, obtenerHoraEspanola } from '@/lib/utils';

export default function ImagesUpload({ upload_limit }: { upload_limit: UploadLimit | null }) {
  ...
  const showForm = () => {
    ...
    if (upload_limit === null ) return true
    
    return upload_limit.can_upload && upload_limit.left > 0
  ...
  return (
      ...
      <div className="flex flex-col gap-4 p-4">
        <Card>
          ...
          <CardContent>
            { !showForm() && upload_limit?.time && <p>Limite de subidas alcanzado. Fin del límite: {formatFechaEspanola(upload_limit?.time ?? '')} {obtenerHoraEspanola(upload_limit?.time ?? '')}</p>}
            { upload_limit?.can_upload == false && <div className="flex flex-row gap-2"><p>No tienes permiso para subir imágenes </p><Frown className="h-6 w-6" /></div>}
            { showForm() &&
              <form onSubmit={submit} className="space-y-4">
                ...
                <div
                  ...
                  {upload_limit && <p className="text-sm text-gray-400">{upload_limit?.left} subidas disponibles</p>}
                  {upload_limit == null && <p className="text-sm text-gray-400">Sin limites de subidas</p>}
                </div>
